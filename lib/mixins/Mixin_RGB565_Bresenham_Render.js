...
const Mixin_RGB565_Bresenham_Render = Base => class extends Base {
    ...
    */
    /**************************************************************************/
    drawRect(x, y, w, h, color, {width = this.draw.width, antialias = this.draw.antialias} = {}) {
        if (!antialias && (width == 1)) {
            super.drawRect(x, y, w, h, color);
        } else {
            this.drawLine(x, y,        x + w - 1, y,         color, {width, antialias});
            this.drawLine(x, y + h -1, x + w - 1, y + h - 1, color, {width, antialias});
            this.drawLine(x,         y, x,         y + h - 1, color, {width, antialias});
            this.drawLine(x + w - 1, y, x + w - 1, y + h - 1, color, {width, antialias});
        }
        return this;
    }

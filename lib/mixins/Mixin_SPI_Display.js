...
const Mixin_SPI_Display = Base => class extends Base {
    ...
    async _dcTransition(newState) {
        const self = this, dcGpio = self._dcGpio;
        if (dcGpio) {
            if (dcGpio.readSync() !== newState) {
                dcGpio.writeSync(newState);
                await sleepMs(1);
            }
            return newState;
        } else {
    ...
    _hardwareWriteCommand(command, data) {
        ...
        const doWork = async _ => {
            ...
            await self._dcTransition(GPIO_LOW);
            await sendSpiBytes(spiConnection, [command]);

            if (data && data.length) {
                if (!dcGpioLowForCommandData) {
                    await self._dcTransition(GPIO_HIGH);
                }
...
async function sendSpiBytes(spiConnection, data = []) {
    if (data && data.length) {
        ...
        while(bytesRemaining) {
            const countToWrite = Math.min(bytesRemaining, blockSize);
            const dataToSend = Buffer.from(data.slice(start, start + countToWrite));
            await _sendSpiBlock(spiConnection, dataToSend);
            bytesRemaining -= countToWrite;
            start += countToWrite;
        }
    }

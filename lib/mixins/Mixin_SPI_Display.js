...
const GpioEngine = require("./GpioEngine_Rpio");
...
const Mixin_SPI_Display = Base => class extends Base {
    ...
    _hardwareStartup() {
        ...
        const doWork = _ => {
            if ((undefined === self._rstGpio) && (self._rstGpioId >= 0)) {
                ...
                self._rstGpio = gpioEngine.createGpioOut(self._rstGpioId, GpioEngine.GPIO_HIGH);
                self._rstGpio && self._rstGpio.startup();
            }
            if ((undefined === self._dcGpio) && (self._dcGpioId >= 0)) {
                gpioEngine.startup();
                // Initialize with high value.
                self._dcGpio = gpioEngine.createGpioOut(self._dcGpioId, GpioEngine.GPIO_HIGH);
                self._dcGpio && self._dcGpio.startup();
            }
            if (undefined === self._spiConnection) {
                ...
                self._spiConnection = openSpiConnection(self._spiBusNumber, self._spiDeviceNumber, self._spiMode, self._spiMaxSpeedHz);
                _debug && console.log("options:%o", self._spiConnection.getOptionsSync());
            }
        };
        self._chain(doWork);
    ...
    _hardwareShutdown() {
        ...
        const doWork = _ => {
            ...
            if (self._rstGpio) {
                self._rstGpio.shutdown();
                self._rstGpio = undefined;
            }
            if (self._dcGpio) {
                self._dcGpio.shutdown();
...
async function gpioHardwareReset_101(gpio) {
    const resetMs = 150;
    if (gpio) {
        gpio.writeSync(GpioEngine.GPIO_HIGH);
        await sleepMs(resetMs);
        gpio.writeSync(GpioEngine.GPIO_LOW);
        await sleepMs(resetMs);
        gpio.writeSync(GpioEngine.GPIO_HIGH);
        await sleepMs(resetMs);
    }

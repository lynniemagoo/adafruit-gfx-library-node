...
{                                          /* plot any quadratic Bezier curve */
   ...
   if (x*(x2-x1) > 0) {                              /* horizontal cut at P4? */
      ...
      plotQuadBezierSeg(dc, x0,y0, x,Math.floor(r+0.5), x,y);
   ...
   if ((y0-y1)*(y2-y1) > 0) {                          /* vertical cut at P6? */
      ...
      plotQuadBezierSeg(dc, x0,y0, Math.floor(r+0.5),y, x,y);
...
{                                 /* plot any quadratic rational Bezier curve */
   ...
   if (xx*(x2-x1) > 0) {                             /* horizontal cut at P4? */
      ...
      plotQuadRationalBezierSeg(dc, x0,y0, x,Math.floor(yy+0.5), x,y, ww);
   ...
   if ((y0-y1)*(y2-y1) > 0) {                          /* vertical cut at P6? */
      ...
      plotQuadRationalBezierSeg(dc, x0,y0, Math.floor(xx+0.5),y, x,y, ww);
...
{                                   /* plot ellipse rotated by angle (radian) */
   ...
   plotRotatedEllipseRect(dc, x-a,y-b, x+a,y+b, (4*zd*Math.cos(angle)));
}
...
{             /* draw a black (0) anti-aliased line on white (ALPHA_MAX) background */
   ...
   for ( ; ; ){                                                 /* pixel loop */
      dc.setPixelAA(x0,y0, ALPHA_MAX*Math.abs(err-dx+dy)/ed);
      e2 = err; x2 = x0;
      if (2*e2 >= -dx) {                                            /* x step */
         if (x0 == x1) break;
...
{                     /* draw a black anti-aliased circle on white background */
   ...
   r = 1-err;
   for ( ; ; ) {
      i = ALPHA_MAX*Math.abs(err+2*(x+y)-2)/r;          /* get blend value of pixel */
...
{        /* draw a black anti-aliased rectangular ellipse on white background */
   ...
   if (--x0 == x1++)                       /* too early stop of flat ellipses */
      while (y0-y1 < b) {
         i = ALPHA_MAX*4*Math.abs(err+dx)/b1;           /* -> finish tip of ellipse */
...
{                    /* draw an limited anti-aliased quadratic Bezier segment */
   ...
   {                                                      /* no straight line */
      ...
      do {
         ...
         dc.setPixelAA(x0,y0, ALPHA_MAX*Math.abs(err-dx-dy-xy)/ed);      /* plot curve */
         if (x0 == x2 || y0 == y2) break;     /* last pixel -> curve finished */
         x1 = x0; cur = dx-err; y1 = 2*err+dy < 0;
         if (2*err+dx > 0) {                                        /* x step */
            if (err-dy < ed) dc.setPixelAA(x0,y0+sy, ALPHA_MAX*Math.abs(err-dy)/ed);
            x0 += sx; dx -= xy; err += dy += yy;
         }
         if (y1) {                                                  /* y step */
            if (cur < ed) dc.setPixelAA(x1+sx,y0, ALPHA_MAX*Math.abs(cur)/ed);
...
{                             /* plot any anti-aliased quadratic Bezier curve */
   ...
   if (x*(x2-x1) > 0) {                              /* horizontal cut at P4? */
      ...
      x = Math.floor(t+0.5); y = Math.floor(r+0.5);
      r = (y1-y0)*(t-x0)/(x1-x0)+y0;                  /* intersect P3 | P0 P1 */
      plotQuadBezierSegAA(dc, x0,y0, x,Math.floor(r+0.5), x,y);
      r = (y1-y2)*(t-x2)/(x1-x2)+y2;                  /* intersect P4 | P1 P2 */
      x0 = x1 = x; y0 = y; y1 = Math.floor(r+0.5);        /* P0 = P4, P1 = P8 */
   }
   if ((y0-y1)*(y2-y1) > 0) {                          /* vertical cut at P6? */
      ...
      x = Math.floor(r+0.5); y = Math.floor(t+0.5);
      r = (x1-x0)*(t-y0)/(y1-y0)+x0;                  /* intersect P6 | P0 P1 */
      plotQuadBezierSegAA(dc, x0,y0, Math.floor(r+0.5),y, x,y);
      r = (x1-x2)*(t-y2)/(y1-y2)+x2;                  /* intersect P7 | P1 P2 */
      x0 = x; x1 = Math.floor(r+0.5); y0 = y1 = y;        /* P0 = P6, P1 = P7 */
   }
...
{   /* draw an anti-aliased rational quadratic Bezier segment, squared weight */
   ...
   if (cur != 0.0 && w > 0.0) {                           /* no straight line */
      ...
      do {                                                      /* pixel loop */
         ...
         x1 = ALPHA_MAX*Math.abs(err-dx-dy+xy)/ed;/* get blend value by pixel error */
         if (x1 < 256) dc.setPixelAA(x0,y0, x1);                   /* plot curve */
         if (f = 2*err+dy < 0) {                                    /* y step */
            if (y0 == y2) return;             /* last pixel -> curve finished */
            if (dx-err < ed) dc.setPixelAA(x0+sx,y0, ALPHA_MAX*Math.abs(dx-err)/ed);
         }
         if (2*err+dx > 0) {                                        /* x step */
...
{                    /* plot any anti-aliased quadratic rational Bezier curve */
   ...
   if (xx*(x2-x1) > 0) {                             /* horizontal cut at P4? */
      ...
      x = Math.floor(xx+0.5); y = Math.floor(yy+0.5);                   /* P4 */
      yy = (xx-x0)*(y1-y0)/(x1-x0)+y0;                /* intersect P3 | P0 P1 */
      plotQuadRationalBezierSegAA(dc, x0,y0, x,Math.floor(yy+0.5), x,y, ww);
      yy = (xx-x2)*(y1-y2)/(x1-x2)+y2;                /* intersect P4 | P1 P2 */
      y1 = Math.floor(yy+0.5); x0 = x1 = x; y0 = y;       /* P0 = P4, P1 = P8 */
   }
   if ((y0-y1)*(y2-y1) > 0) {                          /* vertical cut at P6? */
      ...
      x = Math.floor(xx+0.5); y = Math.floor(yy+0.5);                   /* P6 */
      xx = (x1-x0)*(yy-y0)/(y1-y0)+x0;                /* intersect P6 | P0 P1 */
      plotQuadRationalBezierSegAA(dc, x0,y0, Math.floor(xx+0.5),y, x,y, ww);
      xx = (x1-x2)*(yy-y2)/(y1-y2)+x2;                /* intersect P7 | P1 P2 */
      x1 = Math.floor(xx+0.5); x0 = x; y0 = y1 = y;       /* P0 = P6, P1 = P7 */
   }
...
{                                   /* plot ellipse rotated by angle (radian) */
   ...
   xd = Math.sqrt(xd-zd*s), yd = Math.sqrt(yd+zd*s);       /* surrounding rect*/
   a = Math.floor(xd+0.5); b = Math.floor(yd+0.5); zd = zd*a*b/(xd*yd);
   plotRotatedEllipseRectAA(dc, x-a,y-b, x+a,y+b, (4*zd*Math.cos(angle)));
}
...
exit:   
 ...
 loop:
     for (fx = fy = f; ; ) {
         ...
         y1 = ALPHA_MAX*Math.abs(ex-(f-fx+1)*dx-(f-fy+1)*dy+f*xy)/ed;
...
{                    /* plot any anti-aliased quadratic rational Bezier curve */
   ...
   if (xx*(x2-x1) > 0) {                             /* horizontal cut at P4? */
      ...
      x = Math.floor(xx+0.5); y = Math.floor(yy+0.5);                   /* P4 */
      yy = (xx-x0)*(y1-y0)/(x1-x0)+y0;                /* intersect P3 | P0 P1 */
      plotQuadRationalBezierWidthSeg(dc, x0,y0, x,Math.floor(yy+0.5), x,y, ww, th);
      yy = (xx-x2)*(y1-y2)/(x1-x2)+y2;                /* intersect P4 | P1 P2 */
      y1 = Math.floor(yy+0.5); x0 = x1 = x; y0 = y;       /* P0 = P4, P1 = P8 */
   }
   if ((y0-y1)*(y2-y1) > 0) {                          /* vertical cut at P6? */
      ...
      x = Math.floor(xx+0.5); y = Math.floor(yy+0.5);                   /* P6 */
      xx = (x1-x0)*(yy-y0)/(y1-y0)+x0;                /* intersect P6 | P0 P1 */
      plotQuadRationalBezierWidthSeg(dc, x0,y0, Math.floor(xx+0.5),y, x,y, ww, th);
      xx = (x1-x2)*(yy-y2)/(y1-y2)+x2;                /* intersect P7 | P1 P2 */
      x1 = Math.floor(xx+0.5); x0 = x; y0 = y1 = y;       /* P0 = P6, P1 = P7 */
   }
...
{                                   /* plot ellipse rotated by angle (radian) */
   ...
   xd = Math.sqrt(xd-zd*s), yd = Math.sqrt(yd+zd*s);       /* surrounding rect*/
   a = Math.floor(xd+0.5); b = Math.floor(yd+0.5); zd = zd*a*b/(xd*yd);
   plotRotatedEllipseRectWidth(dc, x-a,y-b, x+a,y+b, (4*zd*Math.cos(angle)), th);
}
